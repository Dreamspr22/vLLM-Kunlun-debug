...
def _fake_rmsnorm(x, weight, output, eps=1e-5, interweave=False,
    ...
    # Set shape/dtype, but do not return value
...
def _fake_gen_block_statistic(
        topk_ids: torch.Tensor,
        block_statistic: torch.Tensor) -> None:
    return None

gen_block_statistic.register_fake(_fake_gen_block_statistic)
...
def moe_pre_sorted(
        ...
        index_have_neg: bool = False) -> None:

    xtorch_ops.moe_pre_sorted(
        x, topk_index, block_statistic, moe_expand, moe_index, expert_m, sorted_tokens_num_lod, index_have_neg)
...
moe_post.register_fake(_fake_moe_post)
...
def moe_fc(
    ...
    xtorch_ops.moe_fc(
        ...
        x_perchannel_max, w_perchannel_max, topk_ids, topk_w,
        bias, tgemm_type, tweight_type, scale_n, scale_k, use_pack_int4)
