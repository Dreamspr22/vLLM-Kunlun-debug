...
if envs.VLLM_USE_FLASHINFER_SAMPLER and find_spec("flashinfer"):
    ...
    from flashinfer.sampling import (
        top_k_top_p_sampling_from_probs as flashinfer_top_k_top_p_sampling)  # yapf: enable
else:
...
def get_sampler() -> torch.nn.Module:
    """
    Get a model for sampling, returning a type of torch.nn.Module.
    If the environment variable VLLM_USE_V1 is set to True, the v1 version of the sampler is used; otherwise, the current version of the sampler is used.

    Returns:
        torch.nn.Module (Union[Sampler, V1Sampler]): A model for sampling, which can be of type Sampler or V1Sampler.
    """
    if envs.VLLM_USE_V1:
...
class SamplerOutput(
    ...
    def __setitem__(self, idx: int, value):
        ...
        Args:
            idx (int): The index of the output to set.
    ...
    def __eq__(self, other: object):
        ...
        Two objects are considered equal if: 1) they are of the same type; 2) they have the same outputs.

        Args:
            other (object): The other object to compare.

        Returns:
            bool: If the two objects are equal, return True; otherwise, return False.
        """
...
def _top_k_top_p_multinomial_with_flashinfer(
    ...
    """
    Use FlashInfer to implement Top-K and Top-P sampling for multiple samples.

    If `seq_groups` is not None, sampling will be performed for each sequence group and the results will be concatenated.

    Args:
        probs (torch.Tensor): The input probability distribution, with shape `(batch_size, vocab_size)`.
        top_ks (torch.Tensor): The Top-K values for each sample, with shape `(batch_size,)`.
        top_ps (torch.Tensor): The Top-P values for each sample, with shape `(batch_size,)`.
        num_samples (int): The number of samples to generate.
        seq_groups (Optional[list[SequenceGroupToSample]]): Optional, a list containing sequence group information, defaults to None.
            SequenceGroupToSample is a class that contains relevant information about a sequence group, including seq_ids and generator (optional).
            If seq_groups is not None, sampling will be performed for each sequence group and the results will be concatenated.

    Returns:
        torch.Tensor: A tensor containing the next token IDs, with shape `(batch_size * num_samples,)`.
