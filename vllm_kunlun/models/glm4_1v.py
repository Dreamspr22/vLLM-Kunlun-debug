...
# Adapted from vllm/model_executor/models/glm4_1v.py
...
from vllm_kunlun.models.qwen2_vl import _qwen2vl_field_config, apply_rotary_pos_emb_vision
...
def grid_sample(input, grid, **kwargs):
    try:
        ...
        # if grid_sample is not implemented on XPU, falling back to CPU.
        result = F.grid_sample(input.cpu(), grid.cpu(), **kwargs).to(device=input.device, dtype=input.dtype).contiguous()
        return result
