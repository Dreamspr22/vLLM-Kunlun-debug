...
class Qwen2_5_VisionRotaryEmbedding(nn.Module):
    ...
    def __init__(self, dim: int, theta: float = 10000.0) -> None:
        ...
        Initialization function to create an instance.
            ...
            Args:
                dim (int): Dimension size, representing the length of the input feature vector.
                theta (float, optional, default=10000.0): Parameter that controls the smoothness of the frequency distribution, default is 10000.0.

            Returns:
                None: Does not return any value, directly creates an instance.
        """
        super().__init__()
        self.dim = dim
        self.theta = theta
...
class Qwen2_5_VisionTransformer(nn.Module):
    ...
    def forward(
        ...
        # Construct rotary_pos_emb_cos_sin_cache to use rope_vit operator
...
class Qwen2_5_VLForConditionalGeneration(
    ...
    def _process_image_input(
        ...
        """
            Process image input and return tensors for each image item.
        If the input is image embeddings, return the image embeddings; otherwise, return the tensors processed by the visual model.

        Args:
            image_input (Qwen2_5_VLImageInputs): A dictionary containing image information, including the following key-value pairs:
                - type (str, optional): The type of the image, which can be "image_embeds" or None (default), indicating the use of image embeddings;
                   If None, the "pixel_values" and "image_grid_thw" key-value pairs must be provided.
                - pixel_values (torch.Tensor, optional): The pixel values of the image, with a shape of (batch_size, num_channels, height, width),
                   dtype is float32, optional, and must be provided only when type is None.
                - image_grid_thw (torch.Tensor, optional): The image grid size, with a shape of (batch_size, 2) and dtype int64,
                   optional, and must be provided only when type is None.

        Returns:
            tuple (torch.Tensor, ...): A tuple containing the tensors for each image item, with the number of tensors equal to the vertical product of the image grid size.
        """
    ...
    def _process_video_input(
        ...
        """
            Process video input and return tensors for each video item.
        If the input is video embeddings, return the video embeddings; otherwise, return the tensors processed by the visual model.

        Args:
            video_input (Qwen2_5_VLVideoInputs): A dictionary containing video information, including the following key-value pairs:
                - type (str): The type of the video, which can be "video_embeds" or "pixel_values_videos".
                - video_grid_thw (torch.Tensor): The video grid size, with a shape of (N, 2), where N is the number of video items, and the last two dimensions represent height and width.
                - video_embeds (Optional[torch.Tensor]): The video embeddings, which are only valid when type is "video_embeds".
                - pixel_values_videos (Optional[torch.Tensor]): The pixel values of the videos, which are only valid when type is "pixel_values_videos".

        Returns:
            tuple[torch.Tensor, ...]: A tuple containing the tensors for each video item, with the number of tensors equal to N.
        """
